...
from utils.loss_utils import l1_loss, ssim, lncc, get_img_grad_weight, local_pearson_loss
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if iteration > opt.depth_reg_weight_from_iter:
            depth_reg_weight = opt.depth_reg_weight
            plane_depth = render_pkg['plane_depth']
            if plane_depth.dim() == 3:
                plane_depth = plane_depth.squeeze(0) # Add batch dimension
                
            if est_depth.shape != plane_depth.shape:
                est_depth = F.interpolate(est_depth, size=plane_depth, mode='bilinear', align_corners=False)
                
            # print(f"Plane depth shape: {plane_depth.shape}, Estimated depth shape: {est_depth.shape}")
            depth_loss = local_pearson_loss(plane_depth, est_depth, 128, 0.5)
            loss += depth_reg_weight * depth_loss.mean()
